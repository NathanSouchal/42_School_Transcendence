...
export default class Account {
  ...
  attachEventListeners() {
    ...
    // Regroupe les boutons utilisant handleChange
    const buttonConfigs = [
    ...
    buttonConfigs.forEach(({ id, action }) => {
      ...
      if (button && !this.eventListeners.some((e) => e.name === action)) {
        const listener = this.handleChange.bind(this, action, "");
    ...
    if (form && !this.eventListeners.some((e) => e.name === "form-submit")) {
      const listener = this.handleChange.bind(this, "form-submit", "");
    ...
    // Événement "change" pour l'upload de l'avatar
    const avatarInput = document.getElementById("avatar");
    if (avatarInput && !this.eventListeners.some((e) => e.name === "avatar")) {
      const listener = (e) => {
        const file = e.target.files[0];
        if (file) this.handleChange("avatar", file);
      };
      avatarInput.addEventListener("change", listener);
      this.eventListeners.push({
        name: "avatar",
        type: "change",
        element: avatarInput,
    ...
    // Écoute les événements "input" sur tous les champs <input>
                ...
                ? `<div id="userinfo-main-div">
                    <form id="user-form">
                      <div class="avatar-main-div" id="avatar-main-div">
                        ${
                          this.userData.avatar
                            ? `<img src="https://127.0.0.1:8000/${this.userData.avatar}">`
                            : `<img src="/profile.jpeg">`
                        }
                        <div class="search-bar-and-result-social">
                          <label class="file-label" for="avatar">Upload file</label>
                      ...
                      <div class="form-username-title-div search-bar-and-result-social" id="username-main-div">
                        ...
                        <input type="text" class="form-control" minLength="4" maxLength="10" value="${this.formData.username}" name="username" required />
                      </div>
                      <div class="form-alias-title-div search-bar-and-result-social" id="alias-main-div">
                        <label for="alias">Alias</label>
                        <input type="text" class="form-control" minLength="4" maxLength="10" value="${this.formData.alias}" name="alias" required />
                      </div>
                      <button type="button" class="btn btn-success m-3" id="update-user-info">Update my info</button>
                    </form>
                  </div>`
                : `<div id="userinfo-main-div">
                    <div class="avatar-main-div" id="avatar-main-div">
                    ...
                    <div class="username-title-div" id="username-main-div">
                      ...
                      <h2 class="username-title-value">${this.userData.username || ""}</h2>
                    </div>
                    <div class="alias-title-div" id="alias-main-div">
                      <h2 class="alias-title">Alias :</h2>
                      <h2 class="alias-title-value">${this.userData.alias || ""}</h2>
                    </div>
                  </div>`
            }
