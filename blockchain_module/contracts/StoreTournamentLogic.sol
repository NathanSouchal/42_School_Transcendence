...
contract StoreTournamentLogic is Initializable {
    ...
    event   TournamentStored(bytes32 tournamentId, address submittedBy);
    ...
    mapping(bytes32 => Round[]) public allTournamentsScores;
    ...
    function _storeRound(Round memory _round, bytes32 _tournamentId) private {
    ...
    function storeFullTournament(Round[] memory rounds, bytes32 tournamentId) public {

        require(rounds.length > 0, "Tournament must have at least one round.");
        require(allTournamentsScores[tournamentId].length == 0, "Tournament ID already exists!");
    ...
    function getTournament(bytes32 tournamentId) public view 
